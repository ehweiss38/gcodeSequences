...
#different increments
   #instead of recalculating, it will g1 if divisible by increment, else g0, saving time while maintaining precision
   # Not exactly sure how this will work in regards to y or what the plan is for that. I dont think it shoul dmatter too much , but may need to change how corners are
...
#note: replaced most instances of increment with 1, because i will still move at 1

from helpers.helpers import modFloor,calcEndpoint,LineBoundaries,setCircleVals,setValues,effectiveVal,relativeMin, movement
...
while xInput==None:
    # is this fine? Yes because it sets limits
    xInput=effectiveVal(1,setValues('x',True,280,oRadius,False))
    if xInput==None:print(f"Value must be greater than outer-radius({oRadius})")
while yInput==None:
    yInput=effectiveVal(1,setValues('y',True,280,oRadius,False))
...
print('y',yMin)
#Adjusts so it respects increments, but idk if it is still necessarry 
#i see a case for either way, but for now i will remove it to retain same precision, even at mild efficiency cost

#changing to one
pathRadius=modFloor(radius-buffer,10)
...
if hBarrier==False:y+=10
print(radius,yTargH)
yTargL=relativeMin(-pathRadius,yMin+10)
...
if(yMax!=0):
    #unsure about here
    f.write(f"G0 X{x} Y{y+10 if hBarrier else y} Z{z} \n\n")
...
while z<zLimit:
    ...
    while(y>=yFinish if downwards else y<=yFinish):
        ...
        lineEnd=calcEndpoint(radius,y,10,buffer)
        ...
        f.write(f"G{movement(x,increment)} X{x} Y{y} (chilipeppr_pause)\n")
        while x<xTarg if rightwards else x>xTarg:
            x+=10 if rightwards else -10
            #if y==yFinish:x=0
            f.write(f"G{movement(x,increment)} X{x} Y{y} (chilipeppr_pause)\n")
    ...
    if z<=zLimit:f.write(f"G{movement(x,increment)} X{x} Y{y} Z{z} \n\n")
...
z-=zIncrement
#moves down
f.write(f"G{movement(x,increment)} Z{z} (chilipeppr_pause)\n")
...
while(z>=relativeMin(-thickness,zMin)):
    #print('Z',z)
    f.write(f"{movement(x,increment)} X{x} Y{yMax} Z{z} (chilipeppr_pause)\n")
    #needs to go
    ...
    while y>=yMin:
        ...
        #needs to change
        line.end=(calcEndpoint(oRadius,y,10,buffer,False) * (1 if xPositive else -1)) if abs(y)<oRadius else (0 if tailSpace==False else (tailCoord if xPositive else -tailCoord))
...
#a little connfusing but low meaninng start, high finish, irrespective of actual values
    ...
    while((downwards and y>=highY) or (downwards==False and y<=highY)):
        ...
        while (x!=highX):
            x+=10 if rightwards else -10
            f.write(f"G{movement(x,increment)} X{x} (chilipeppr_pause)\n")
            #difference between approach direction
        #end behavior
        rightwards=False if rightwards else True
        if y!=highY:y+=-10 if downwards else 10
        else: break
        f.write(f"G{movement(x,increment)} X{x} Y{y} Z{z} (chilipeppr_pause)\n")
